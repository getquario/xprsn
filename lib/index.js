...
let lex = (s) => {
  ...
  for (TOKEN.lastIndex = 0; TOKEN.lastIndex < length;) {
    ...
    if (t === '"' || t === "'")
      throw fault(SyntaxError, "Unexpected " + t, "XPRSN_SYNTAX", at, length);
        ...
        : op === "/"
          ...
          : op === "%"
            ...
            : op === "**"
              ? a ** b
              : op === "~"
                ? "" + a + b // string concat, coerces both sides
                : op === "=="
                  ? a === b
                  : op === "!="
                    ? a !== b
...
let primary = () => {
  ...
  if (ID.test(token)) {
    if (eat("(")) {
      ...
      // oxlint-disable-next-line no-unused-expressions
      Object.hasOwn(fns, token) ||
        err(token + " is not a function", "XPRSN_UNKNOWN_FUNCTION", start);
      functions.add(token);
      let fn = fns[token],
        args = list(")");
...
let postfix = () => {
  ...
  for (;;) {
    ...
    e = step(
      ...
      start,
      args ? pos[i - 1] + toks[i - 1].length : end,
      names,
    );
  }
};
      ...
      op === "and" || op === "&&"
        ...
        : op === "or" || op === "||"
          ...
          : op === "??"
            ...
            : (
                (operator, lhs) => (scope) =>
                  apply(operator, lhs(scope), right(scope))
              )(op, left);
  }
  return left;
};
