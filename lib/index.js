...
/** Unary wrapper. @param {(x: any) => any} f @returns {(e: Node) => Node} */
let un = (f) => (e) => (scope) => f(e(scope));
/** @type {Record<string, (e: Node) => Node>} */
const UNARY = table({ "!": un((x) => !x), "-": un((x) => -x), "+": un((x) => +x) });

/** Constant node. @param {any} v @returns {Node} */
let konst = (v) => () => v;
/** @type {Record<string, Node>} */
const LITERALS = table({ true: konst(!0), false: konst(!1), null: konst(null) });
...
/**
 * One postfix step off base `o`. `access` locates the member key node and its
 * span; `key(scope)` is the member key; `opt` (the `?.` form) yields null on a
 * nullish base instead of throwing, per step; a truthy `args` makes it a
 * method call bound to the base.
 *
 * @param {Node} o
 * @param {boolean} opt
 * @param {Node[] | 0} args
 ...
 */
let step =
  (o, opt, args, [key, start, end], callEnd, own) =>
  (scope) => {
    let b = o(scope);
...
/** @param {number} opAt @returns {Access} */
let indexAccess = (opAt, key = ternary()) => (expect("]"), [key, pos[opAt], endPos(i - 1), 1]);

/** @returns {Access} */
let propAccess = (at = i, k = take(ID)) => [() => k, pos[at], endPos(at), 0];
...
let attach = (e, opt, access) => {
  ...
  let args = /** @type {Node[] | 0} */ (!access[3] && eat("(") ? seq(")", ternary) : 0);
  return step(e, opt, args, access, args ? endPos(i - 1) : access[2], names);
};
