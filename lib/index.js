...
let ident = (token, start) => {
  if (eat("(")) {
    ...
    let fn = fns[token],
      args = list(")");
    return (scope) => fn(...evalAll(args, scope));
  }
  // oxlint-disable-next-line no-unused-expressions
...
/** @param {Node[]} items @param {any} scope */
let evalAll = (items, scope) => items.map((e) => e(scope));
...
let array = () => {
  let items = list("]");
  return (scope) => evalAll(items, scope);
};
...
let invoke = (m, b, args, callStart, callEnd, own, scope) => {
  ...
  return m.apply(b, evalAll(args, scope));
};
...
let step = (o, key, opt, args, start, end, callStart, callEnd, own) => (scope) => {
  ...
  return args ? invoke(m, b, args, callStart, callEnd, own, scope) : m;
};

/** @param {number} opAt */
let indexAccess = (opAt) => {
  let key = ternary();
  expect("]");
  return /** @type {Access} */ ([key, pos[opAt], endPos(i - 1), 1]);
};
...
/** @returns {Node} */
let unary = () => {
  let wrap = UNARY[toks[i]];
  if (!wrap) return postfix();
  i++;
  return wrap(unary());
};
...
let binary = (op, lhs, right) => {
  let short = SHORT[op];
  return short ? short(lhs, right) : (scope) => OPS[op](lhs(scope), right(scope));
};
...
let elvis = (c) => {
  let e = ternary();
  return (/** @type {any} */ scope) => c(scope) || e(scope);
};
...
let conditional = (c) => {
  ...
  expect(":");
  let e = ternary();
  return (/** @type {any} */ scope) => (c(scope) ? t(scope) : e(scope));
};

/** @param {Node} c */
let afterQ = (c) => (eat(":") ? elvis(c) : conditional(c));
...
export function compile(src, funcs, opts) {
  ...
  try {
    ...
    if (!(x instanceof RangeError)) throw x;
    throw fault(SyntaxError, "Expression too deeply nested", "XPRSN_TOO_DEEP", 0, length);
  }
