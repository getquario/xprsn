...
let ident = (token, start) => {
  if (eat("(")) {
    ...
    functions.add(token);
    // A promise is the one return an embedder can still do something with, and
    // only when this call is the whole expression -- anywhere else an operator
    // consumes it unawaited, which is silent and always wrong. Whether it is
    // the whole expression is known once the parse ends, so the node reads a
    // cell `compile` marks rather than a value it could capture here.
    let fn = fns[token],
      args = seq(")", ternary),
      at = pos[start],
      to = endPos(i - 1),
      ...
      node = /** @type {any} */ (
        (/** @type {any} */ scope) => {
          let out = fn(...args.map((e) => e(scope)));
          return whole.root || typeof out?.then !== "function"
            ? out
            : fault(
                TypeError,
                token + " returned a promise, and nothing here can await it",
                "XPRSN_PENDING_VALUE",
                at,
                to,
                own,
...
/**
 * `e` is a default parameter: it consumes the operator and parses the operand
 * only when `op` matched, so a plain postfix never advances `i`.
 *
 * @param {any} [op]
 * @param {any} [e]
 * @returns {Node}
 */
let unary = (op = UNARY[toks[i]], e = op && (i++, unary())) =>
  op ? (scope) => op(e(scope)) : postfix();
...
export function compile(src, funcs, opts) {
  ...
  // Only here is the root node known, so this is where a call that is the
  // whole expression learns it may hand its promise back.
  if (/** @type {any} */ (e).whole) /** @type {any} */ (e).whole.root = true;
