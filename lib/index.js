 ...
 * line that throws it rather than shipping.
 *
 ...
 *
 * This module's store applied to waarmerk's relocation, which is where the copy
 * rule and the authentication it has to satisfy live together. `offset` shifts
 * the span, for an embedder that handed over a verbatim slice of its own text;
 * `span` replaces it, for one whose text reached the expression through a
 * decode and has no linear offset to shift.
 *
...
/**
 * Throw a located diagnostic. `const`, not `let`: TypeScript only propagates a
 * `never` return through control-flow analysis when the callee binding cannot
 * be reassigned, and every caller here relies on this one ending a branch.
 *
...
/** @type {Bin} */
let AND = (lhs, right) => (scope) => lhs(scope) && right(scope);
/** @type {Bin} */
let OR = (lhs, right) => (scope) => lhs(scope) || right(scope);

/**
 * Binary operators: `[precedence, builder]` (higher binds tighter). `**` is
 * right-associative. `~` (string concat, coerces both sides) sits below
 * comparison and above `+`, so `"x: " ~ a + b` joins the sum and
 * `a ~ b == "12"` compares the joined string. `and`/`or`/`??` build
 * short-circuiting nodes directly; the rest wrap a strict `(a, b)` op via
 * `bin`. Looked up by arbitrary token text; a miss fails the `>= min`
 * precedence test in `expr` before the builder is ever read.
 *
 ...
 * @returns {never}
 */
const err = (msg, at = i, code = "XPRSN_SYNTAX") => {
  let t = toks[at],
    p = t ? pos[at] : length;
...
let get = (o, k, start, end, own) => {
  ...
  // Coerce once, then guard and read with that same string: a key whose
  // toString() is attacker-supplied would otherwise slip past an identity test
  // and only become "constructor" on the o[k] below. String() rather than
  // "" + k so a symbol stringifies instead of throwing an uncoded TypeError.
  k = String(k);
  if (BLOCKED.has(k))
...
/** Evaluate compiled argument/item nodes. @param {Node[]} args @param {any} scope */
let spread = (args, scope) => args.map((e) => e(scope));
...
/**
 * A token whose value is spelled out: a string or a number. The keyword
 * literals are rows in HEAD instead, so this is only the two patterns.
 *
 * @param {string} token
 * @param {number} start
 * @returns {Node | null}
...
let literal = (token, start) =>
  /^["']/.test(token) ? konst(str(token, start)) : /^[\d.]/.test(token) ? konst(+token) : null;
...
let ident = (token, start) => {
  if (eat("(")) {
    ...
    let fn = fns[token],
      args = seq(")", ternary);
    return (scope) => fn(...spread(args, scope));
  }
  // oxlint-disable-next-line no-unused-expressions
...
let invoke = (m, b, args, callStart, callEnd, own, scope) => {
  ...
  return m.apply(b, spread(args, scope));
};
...
/**
 * `o` stepped through the member access that follows it, or null when none
 * does: `[expr]` is a computed key spanning from the operator token `opAt`,
 * `.name` (and the `?.` form) a literal one spanning the name alone. `at`/`k`
 * are locals, held as parameters so the whole production stays one expression.
 *
 * @param {Node} o
 * @param {number} opAt
 * @param {boolean} opt
 ...
 * @returns {Node | null}
 */
let accessAt = (o, opAt, opt, at, k) =>
  eat("[")
    ? ((k = ternary()), expect("]"), step(o, opt, k, pos[opAt], endPos(i - 1), 1))
    : opt || eat(".")
      ? ((at = i), (k = take(ID)), step(o, opt, () => k, pos[at], endPos(at), 0))
      : null;
...
let postfix = () => {
  let e = primary(),
    ...
    opt,
    next;
  while (((opAt = i), (opt = eat("?.")), (next = accessAt(e, opAt, opt)))) e = next;
  return e;
};
...
/**
 * The tail after `?`: `a ?: b` yields the condition when truthy, else `b`;
 * `a ? b : c` selects. Elvis parses no `t`, and keeps `||` so `c` runs once.
 *
 * @param {Node} c
 * @param {Node | 0} [t]
 * @param {Node} [e]
 */
let branch =
  (c, t = eat(":") ? 0 : ternary(), e = (t && expect(":"), ternary())) =>
  (/** @type {any} */ scope) =>
    t ? (c(scope) ? t(scope) : e(scope)) : c(scope) || e(scope);
