...
let primary = () => {
	...
	if (ID.test(t)) {
		if (eat('(')) {
			...
			Object.hasOwn(fns, t) || err(t + ' is not a function', 'XPRSN_UNKNOWN_FUNCTION', at); // oxlint-disable-line no-unused-expressions
			fnm.add(t);
			let fn = fns[t], args = list(')');
			return v => fn(...args.map(e => e(v)));
		}
		bnd.has(t) || nm.add(t); // oxlint-disable-line no-unused-expressions
		let o = nm;
...
let lambda = () => {
	...
	bnd.add(n);
	let b = ternary();
	had || bnd.delete(n); // oxlint-disable-line no-unused-expressions
