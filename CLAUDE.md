...
4. **Size is a soft goal (~1.2KB min+gzip).** Repetition over abstraction and data-driven operator tables still earn their keep. **Short identifiers do not** — a consumer's minifier mangles every binding regardless of how it is spelled here, so `scope` costs exactly what `v` costs, and renaming the whole file measured _smaller_ because longer repeated names compress better. Since `lib/` ships verbatim, these names are what a consumer reads in a stack trace, so name bindings for readers. Object _property_ names are the exception, being unmanglable — spend those bytes deliberately. Never trade a safety guard or a passing test for bytes. Check size impact with `npm run size`, which bundles and minifies `lib/` the way a consumer's bundler would.
