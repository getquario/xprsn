...
test("prototype escape hatches are blocked", () => {
  ...
  assert.throws(() => evaluate('a["constructor"]', { a: {} }), TypeError);
  assert.throws(
    () => evaluate("a[key]", { a: {}, key: "constructor" }),
    TypeError,
    "computed key resolved at runtime",
  );
  ...
  assert.throws(
    ...
    TypeError,
    "bare variable lookup is guarded too",
  );
  assert.throws(
    () => evaluate("$.constructor", { $: {} }),
    TypeError,
    "the $ anchor does not bypass the guard",
  );
  assert.throws(
    () => evaluate('@["__proto__"]', { "@": {} }),
    TypeError,
  ...
  assert.throws(
    () => evaluate("a?.constructor", { a: {} }),
    TypeError,
    "null-safe access is still guarded",
  );
  assert.throws(() => evaluate('a?.["constructor"]', { a: {} }), TypeError);
  assert.strictEqual(
    evaluate("a?.constructor", { a: null }),
    null,
    "nullish base short-circuits before the key is touched",
  );
...
test("null normalization does not relax the guards", () => {
  assert.throws(
    () => evaluate("a.constructor", { a: {} }),
    TypeError,
    "prototype keys still blocked",
  );
  assert.throws(
    () => evaluate("a.b", { a: null }),
    TypeError,
    "reading through a null base still throws",
  );
  assert.throws(
    () => evaluate("a.b.c", { a: {} }),
    TypeError,
    "missing key becomes null, then reading through it throws",
  );
});
...
test("functions only resolve from the registry", () => {
  ...
  assert.throws(
    () => evaluate("boom()", { boom: () => 1 }),
    SyntaxError,
    "values are not callable as functions",
  );
});
...
test("lambda values cannot reach the Function constructor", () => {
  ...
  assert.throws(
    () => evaluate("(x => x).constructor"),
    TypeError,
    "constructor blocked on a lambda value",
  );
  assert.throws(
    () => evaluate('(x => x)["constructor"]'),
    TypeError,
    "computed constructor blocked too",
  );
  assert.throws(
    () => evaluate("(x => x).__proto__"),
    TypeError,
    "__proto__ (Function.prototype) blocked",
  );
  ...
  assert.throws(
    () => evaluate("(x => x).call.constructor"),
    TypeError,
    "call chain terminates at the guard",
  );
  assert.throws(
    () => evaluate("(x => x).bind.constructor"),
    TypeError,
    "bind chain terminates at the guard",
  );
...
test("a lambda param cannot be invoked as a function", () => {
  ...
  const reducers = { map: (a, f) => a.map((x) => f(x)) };
  assert.throws(
    () => compile("map(a, f => f(f))", reducers),
    SyntaxError,
    "calling the param is a compile-time error",
  );
});
...
test("a param named for a blocked key stays inert", () => {
  ...
  const before = {}.polluted;
  assert.throws(
    () => evaluate("map(rows, __proto__ => __proto__.x)", { rows: [{ x: 1 }] }, reducers),
    TypeError,
    "reading a __proto__ param is blocked",
  );
  assert.throws(
    () => evaluate("map(rows, constructor => constructor.x)", { rows: [{ x: 1 }] }, reducers),
    TypeError,
    "reading a constructor param is blocked",
  );
